<?php
header("Content-Type: application/json");
header("Access-Control-Allow-Origin: *");

/** Connect to Database **/
$db = mysqli_connect("localhost", "root", "", "classicmodels");
if(mysqli_connect_errno($db)) {
	die("Database Connect Error: " + mysqli_connect_error());
}

/** Get Session Key and Token **/
$sql_token = "SELECT Ks, token FROM users WHERE id = '" . $_GET["ID"]."'";
$rs_token = mysqli_query($db, $sql_token);
if(mysqli_num_rows($rs_token) != 0) {
$row = mysqli_fetch_assoc($rs_token);
$Ks = $row["Ks"];
$T = $row["token"];

/** Check Message Authentication **/
$message = $_GET["ID"] . $_GET["TOp"] . $_GET["officeId"];
$mac = $_GET["MAC"];
if(hash("sha256", $message . $Ks) == $mac) {

	/** Token Validation **/
	if($_GET["TOp"] == $T + 1) {
		/** Set the new Token **/ 
		$T  = mt_rand();
		$sql_T = "UPDATE users SET token = " . $T . " WHERE id = '" . $_GET["ID"]."'";
		$rs_T = mysqli_query($db, $sql_T);
		if($rs_T === FALSE) {
			/** Handle Exception **/
			echo "UPDATE new T ERROR!";
		}
		else {
			/** Encode the New Token with XOR **/
			$EKsT = "";
			for($i = 0; $i < strlen($T); $i++) {
				$char = ord(substr($T,$i)) ^ ord(substr($Ks, $i));
				if(strlen($char) == 1) {
					$char = "00" . $char;
				};
				if(strlen($char) == 2) {
					$char = "0" . $char;
				};
				$EKsT .= $char;
			}

			/** Get the data **/
			$sql = "SELECT * FROM offices WHERE officeCode = " . $_GET["officeId"];
			$rs = mysqli_query($db, $sql);
			$office = array();
			$transform = "";
			if(mysqli_num_rows($rs) != 0) {
				$row = mysqli_fetch_assoc($rs);
				$office["officeCode"] = $row["officeCode"];
				$transform .= $row["officeCode"];
					
				$office["city"] = $row["city"];
				$transform .= $row["city"];
					
				$office["phone"] = $row["phone"];
				$transform .= $row["phone"];
					
				$office["addressLine1"] = $row["addressLine1"];
				$transform .= $row["addressLine1"];
					
				$office["addressLine2"] = $row["addressLine2"];
				$transform .= $row["addressLine2"];
					
				$office["state"] = $row["state"];
				$transform .= $row["state"];
					
				$office["country"] = $row["country"];
				$transform .= $row["country"];
					
				$office["postalCode"] = $row["postalCode"];
				$transform .= $row["postalCode"];
					
				$office["territory"] = $row["territory"];
				$transform .= $row["territory"];
			}
			else {
			 	/** Handle Exceptiopn **/
				echo "Erro SELECT Token";
			}
			//echo $transform;
			/** Close connection to Database **/
			mysqli_close($db);
					
			/** Sign **/
			/*$key_path = "/home/barretto/keys/512/private_key_512_pkcs8.pem";
			$fp = fopen($key_path,"r");
			$private_key = fread($fp, filesize($key_path));
			fclose($fp);*/		
			$private_key = "-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----";
			$pkeyid = openssl_get_privatekey($private_key);
			openssl_sign($transform, $sig, $pkeyid);
			openssl_free_key($pkeyid);
			//echo bin2hex($sig);

			/** The Response **/
			$response["ID"] = $_GET["ID"];
			$response["TOp"] = $_GET["TOp"]; //The previous Token
			$response["RS"] = $office;
			$response["EKsT"] = $EKsT; // The next Token
			$response["signature"] = bin2hex($sig);
			echo json_encode($response);
		}
	}
	else {
			/** Handle Exception **/ 
			echo "Token Invalid!";
	}
}
}
else {
	/** Handle Exception **/
	echo "Integrity Invalid!";
}