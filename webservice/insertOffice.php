	...
	if(hash("sha256", $message . $Ks) == $mac) {
		...
		if($_POST["TOp"] == $T + 1) {
			...
			if($rs_T === FALSE) {
				...
				/** Insert the data **/
				$office = json_decode($_POST["D"], true);
				$sql = "INSERT INTO offices VALUES ('" . $office["officeCode"] . "',";
				$sql .=  "'" . $office["city"] . "',";
				$sql .= " '" . $office["phone"] . "', ";
				$sql .= " '" . $office["addressLine1"] . "', ";
				$sql .= " '" . $office["addressLine2"] . "', ";
				$sql .= " '" . $office["state"] . "', ";
				$sql .= " '" . $office["country"] . "', ";
				$sql .= " '" . $office["postalCode"] . "', ";
				$sql .= " '" . $office["territory"] . "') ";
				//echo $sql;
